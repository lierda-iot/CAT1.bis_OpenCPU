...
用法:
    python3 gen_web_asset.py <www_dir> <out_c>

生成 liot_router_resource.c，内含每个文件的 gzip 数据与索引表 gWebAssets[]。
httpd 静态 handler 按请求路径查表返回，带 Content-Encoding: gzip。
配套的 liot_router_resource.h 为固定头文件，手工维护于 api/ 目录，不再自动生成。
...
def main():
    ...
    www, out_c = sys.argv[1], sys.argv[2]
