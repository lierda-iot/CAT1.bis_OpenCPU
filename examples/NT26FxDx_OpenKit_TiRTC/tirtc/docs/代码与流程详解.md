...
> 适用目录：`examples/NT26FxDx_OpenKit_TiRTC/tirtc`
...
1. 设置 TiRTC 源码和 SDK 路径，并为每个 `HWDEMO_*_EN` 选择 `.o` 和编译宏。
2. 在 make 配置阶段检查依赖，防止生成“能链接但运行必坏”的半功能固件。
3. 加入 `libTiRTC.a`、`-fno-lto` 和 TiRTC ABI wrapper。
4. 从 F6D_A 底包 `ap_lierda_app.elf` 解析 `netif_list` 地址；找不到时立即停止构建。

项目入口 `../Makefile` 负责选择板级 `iodriver.ini`、加载项目 `config`，并由
`MODEM=NT26F6D0` 自动匹配 `MODEMPKG=F6D_A`。memory map 由 SDK 公共规则生成。
