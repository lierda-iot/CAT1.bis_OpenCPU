...
'''
    ...
    if lvgl8_compat:
        lvgl8_cf_map = {
            ...
            ColorFormat.RAW_ALPHA: "LV_IMG_CF_RAW_ALPHA",
            ColorFormat.RGB565: "LV_IMG_CF_TRUE_COLOR",
        }
        if cf not in lvgl8_cf_map:
            raise ParameterError(f"LVGL8 C array export not supported for {cf.name}")
        ending = f'''
}};
...
class LVGLCompressData:
    ...
    def _compress(self, raw_data: bytes) -> bytearray:
        ...
        if self.compress == CompressMethod.RLE:
            ...
            compressed = require_lz4().block.compress(raw_data, store_size=False)
        else:
...
class LVGLImage:
    ...
    def to_png(self, filename: str):
        ...
        if self.cf.is_indexed:
            ...
            encoder = require_png().Writer(self.w,
                                 self.h,
                                 palette=palette,
            ...
            for a in transparency:
                data += [0, 0, 0, a]
            encoder = require_png().Writer(self.w, self.h, greyscale=False, alpha=True)
    ...
    def _png_to_indexed(self, cf: ColorFormat, filename: str):
        ...
        png_mod = require_png()
        reader = png_mod.Reader(filename=filename)
        ...
        if cf == ColorFormat.I8:
            ...
            for e in png_mod.pack_rows(rows, cf.bpp):
                rawdata += e
    ...
    def _png_to_alpha_only(self, cf: ColorFormat, filename: str):
        png_mod = require_png()
        reader = png_mod.Reader(str(filename))
        ...
        if cf == ColorFormat.A8:
            ...
            for row in png_mod.pack_rows(rows, cf.bpp):
                rawdata += row
    ...
    def _png_to_luma_only(self, cf: ColorFormat, filename: str):
        reader = require_png().Reader(str(filename))
        w, h, rows, info = reader.asRGBA8()
        rawdata = bytearray()
        for row in rows:
            R = row[0::4]
...
class RAWImage():
    ...
    def __init__(self,
                 cf: ColorFormat = ColorFormat.UNKNOWN,
                 data: bytes = b'',
                 lvgl8_compat: bool = False) -> None:
        self.cf = cf
        self.data = data
        self.lvgl8_compat = lvgl8_compat
    ...
    def to_c_array(self,
        ...
        write_c_array_file(0, 0, 0, self.cf, filename,
                           False, CompressMethod.NONE, self.data, self.lvgl8_compat)
    ...
    def convert(self):
        ...
        for f in self.files:
            if self.cf in (ColorFormat.RAW, ColorFormat.RAW_ALPHA):
                ...
                img = RAWImage(lvgl8_compat=self.lvgl8_compat).from_file(f, self.cf)
                ...
                if self.ofmt == OutputFormat.BIN_FILE:
                    ...
                    img.to_c_array(self._replace_ext(f, ".c"),
                                   compress=self.compress,
                                   lvgl8_compat=self.lvgl8_compat)
...
def main():
    ...
    parser.add_argument('--lvgl8', action='store_true',
                        help="emit LVGL8-compatible C arrays when writing .c output",
                        default=False)
    parser.add_argument(
    ...
    if path.isfile(args.input):
        ...
        if cf in (ColorFormat.RAW, ColorFormat.RAW_ALPHA):
            ...
            for pattern in patterns:
                files.extend(Path(args.input).rglob(pattern))
        else:
