...
typedef struct
{
    ...
    uint8_t codecType;           /*!< Liot_AudDevice_e; library ABI uses one byte */
    uint8_t channel;             /*!< Liot_AudChannel_e; library ABI uses one byte */
    uint8_t role;                /*!< Liot_AudRole_e; library ABI uses one byte */
    uint8_t mode;                /*!< Liot_AudMode_e; library ABI uses one byte */
    uint8_t frameSize;           /*!< Liot_AudFrameSize_e; library ABI uses one byte */
    uint8_t samples;             /*!< Liot_AudSample_e; library ABI uses one byte */
